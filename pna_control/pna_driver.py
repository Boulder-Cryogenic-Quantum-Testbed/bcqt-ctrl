...
def get_data(centerf: float, 
            ...
            #  instr_addr : str = 'GPIB::16::INSTR', # If using GPIB  7/18
             instr_addr : str = 'TCPIP0::K-N5231B-57006.local::inst0::INSTR',
             sparam : str = 'S21',
             cal_set : str = None,
...
def name_datafile(power: float = None,
    ...
    if type(temp) == tuple or type(temp) == list:   temp = float(*temp)
    if type(power) == tuple or type(power) == list:   power = float(*power)
    if type(freq) == tuple or type(freq) == list:   freq = float(*freq)
