...
alL_folders = sorted([x for x in glob.glob(f"{search_dir}/*GHz")])

# if looking through all datasets, filter through 
if search_dir == 'data':
    chosen_timestamp = -1
    all_timestamped_folders = sorted([x for x in glob.glob(f"{search_dir}/*") if 'all_plots' not in x])
    chosen_folder = all_timestamped_folders[chosen_timestamp]
    alL_folders = sorted([x for x in glob.glob(f"{chosen_folder}/*GHz")])
...
for resonator_folder_path in chosen_resonators:
    ...
    print(len(glob.glob(f"{resonator_folder_path}/*")))
...
for resonator_folder_path in chosen_resonators:
    ...
    all_names, all_paths = hl.load_files_in_dir(resonator_folder_path, key="*.csv", blacklist='qiqc', debug=True)   
